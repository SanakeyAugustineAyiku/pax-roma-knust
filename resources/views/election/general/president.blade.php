...
@if ($candidates && count($candidates) > 0)
    ...
    <div class="center">
        {{-- <a href="{{route("election.vote.vp",[$candidate->election_category,$candidate->election_name])}}" class="btn">previous</a> --}}
        <a href="{{route("election.vote.vp",[$candidate->election_category,str_replace("/","_",$candidate->election_name)])}}" class="btn">next</a>
    </div>
 @endif
@endsection
